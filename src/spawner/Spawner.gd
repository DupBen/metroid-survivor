...
func get_random_position():
	var viewport = get_viewport_rect().size * randf_range(1.4, 1.8)

	var viewport_x_half = viewport.x / 2
	var viewport_y_half = viewport.y / 2
	var player_pos = player.global_position
	
	# TODO spawning works now, tho if samus is moving theres a slight chance an enemy can spawn on screen.
	# Need to account for movement buffers
